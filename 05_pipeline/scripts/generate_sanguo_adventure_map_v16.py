...
DEFAULT_OUTPUT = REPO_ROOT / "graphrag-demo/三國演義探險地圖_v16.html"
...
def build_payload(nodes_path: Path, rels_path: Path, metadata_path: Path) -> dict[str, Any]:
    ...
    return {
        "version": "v16-battle-and-path-learning-layout",
        "title": "三國演義探險地圖",
        "generatedFrom": {
  ...
  (() => {
    ...
    function updateScenarioChrome() {
      ...
      dom.relationReaderHint.textContent = state.mode === "battle"
        ? "照原因、決策、行動、結果讀證據。"
        : (state.mode === "relation" ? "每一段都標出戰役、互動或身分線索。" : "選一種關係，看誰和他最有關。");
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Sanguo Adventure Map v16 HTML.")
